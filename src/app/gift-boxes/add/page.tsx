...
export default function AddGiftBox() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to create gift box');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Add Gift Box</h1>
          <p className="text-slate-500 mt-1">Create a new gift box configuration.</p>
        </div>
      </div>

      {success && (
        <div className="bg-green-50 text-green-700 p-4 rounded-xl flex items-center space-x-3 border border-green-200">
          <CheckCircle2 size={20} /><span className="font-bold text-sm">Gift box created successfully!</span>
        </div>
      )}
      {error && (
